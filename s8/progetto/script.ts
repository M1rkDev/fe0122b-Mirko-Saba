...
.then(vestiti => {
    ...
    vestiti.forEach((item: Abbigliamento) => {
        ...
        let capi = new Abbigliamento(item.id, item.codprod, item.collezione, item.capo, item.modello, item.quantita, item.colore, item.prezzoivaesclusa, item.prezzoivainclusa, item.disponibile, item.saldo)
        console.log(capi)
...
let primoCapo = new Abbigliamento(6,1003,"estate","camicia",2522,26,"blue",120,146.4,"magazzino",30)
...
let secondoCapo = new Abbigliamento(7, 2020, "inverno","giacca",2000,30,"nero",170,207.4,"negozio", 15)
...
let terzoCapo = new Abbigliamento(8, 1558, "autunno","jeans",1942,12,"rossi",30,36.6,"negozio", 50)
